...
{
	eDebug("[eAMLTSMPEGDecoder] SETTING UP DECODER                   ------------ AML");
	if (m_demux)
...
{
	eDebug("[eAMLTSMPEGDecoder] CLOSING DONW DECODER                   ------------ AML");
	if (m_radio_pic_on)
		finishShowSinglePic();
	...
	adec_handle = NULL;
	if (aml_fd >= 0) {
		eDebug("[eAMLTSMPEGDecoder::destructor] closing aml_fd=%d   --------- AML !!!!!", aml_fd);
		close(aml_fd);
	}
